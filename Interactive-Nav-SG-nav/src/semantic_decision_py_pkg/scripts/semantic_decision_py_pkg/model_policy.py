...
def compact_graph(graph: dict[str, Any], max_nodes: int = 80, max_edges: int = 160) -> dict[str, Any]:
    ...
    for node in graph_nodes:
        ...
        compact_node.update(aabb_center=list(node.get("aabb_center") or [])[:3],
                            aabb_size=list(node.get("aabb_size") or [])[:3],
                            confidence=node.get("confidence"), observation_count=node.get("observation_count"),
                            last_seen=node.get("last_seen"), attributes=public_attributes)
