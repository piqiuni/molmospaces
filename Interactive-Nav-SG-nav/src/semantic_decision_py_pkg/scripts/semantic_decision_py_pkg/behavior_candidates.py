...
class CandidateGenerator:
    ...
    def _room_context_for_xy(
        ...
        return room_context_for_xy(graph, xy)
    ...
    def _interaction_candidates(
        ...
        for node in graph.get("nodes") or []:
            ...
            if (
                ...
                and (room_segment_grid is not None or any(
                    item.get("type") == "room" for item in graph.get("nodes", [])
                ))
            ):
    ...
    def _approach_candidates(
        ...
        *,
        ...
        if node_type == "portal":
            if (not self.config.portal_allow_opposite_side_interaction
                    and not self.config.portal_require_reference_yaw):
    ...
    def _portal_approach_pose(
        self, robot_xy, target_xy, node, standoff_m,
        side_multiplier=1.0, tangent_offset_m=0.0,
    ) -> list[float]:
        portal_id = str(node.get("id") or "")
        pose, side = portal_approach_pose(
            robot_xy, target_xy, node, standoff_m,
            side_multiplier=side_multiplier, tangent_offset_m=tangent_offset_m,
            previous_side=self._portal_side_sign_by_id.get(portal_id),
            side_hysteresis_m=self.config.portal_side_hysteresis_m,
        )
        if side is not None:
