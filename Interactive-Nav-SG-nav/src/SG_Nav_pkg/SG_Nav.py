...
# 添加项目根目录到路径（用于导入外部依赖，必须在导入 GLIP 之前）
...
try:
    ...
    from SG_Nav_pkg.utils.utils_glip import *
...
class SG_Nav_Agent():
    def __init__(self, task_config, args=None):
        ...
        ### ------ init glip model ------ ###
        config_file = os.path.join(self.project_root, "GLIP/configs/pretrain/glip_Swin_L.yaml")
        weight_file = os.path.join(self.project_root, "GLIP/MODEL/glip_large_model.pth")
        ...
        self.co_occur_mtx = np.load(os.path.join(self.project_root, 'tools/obj.npy'))
        self.co_occur_mtx -= self.co_occur_mtx.min()
        self.co_occur_mtx /= self.co_occur_mtx.max() 
        
        self.co_occur_room_mtx = np.load(os.path.join(self.project_root, 'tools/room.npy'))
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.abspath(os.path.join(script_dir, '../..'))
        self.visualization_dir = os.path.join(self.project_root, f'data/visualization/{self.experiment_name}/')
