...
# 确保 segment_anything 目录在路径中
segment_anything_path = os.path.join(project_root, 'segment_anything')
if segment_anything_path not in sys.path:
    sys.path.insert(0, segment_anything_path)

# 延迟导入 segment_anything，避免 torchvision 问题
try:
    from segment_anything import SamAutomaticMaskGenerator, SamPredictor, sam_model_registry
except ImportError as e:
    # 如果导入失败，尝试从 segment_anything 目录直接导入
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        "segment_anything",
        os.path.join(segment_anything_path, "segment_anything", "__init__.py")
    )
    if spec and spec.loader:
        segment_anything_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(segment_anything_module)
        SamAutomaticMaskGenerator = segment_anything_module.SamAutomaticMaskGenerator
        SamPredictor = segment_anything_module.SamPredictor
        sam_model_registry = segment_anything_module.sam_model_registry
    else:
        raise ImportError(f"无法导入 segment_anything: {e}")
...
try:
    ...
    from SG_Nav_pkg.utils.utils_scenegraph.mapping import compute_spatial_similarities, merge_detections_to_objects
    from SG_Nav_pkg.utils.utils_scenegraph.slam_classes import MapObjectList
    from SG_Nav_pkg.utils.utils_scenegraph.utils import filter_objects, gobs_to_detection_list, text2value
    from SG_Nav_pkg.utils.utils_scenegraph.grounded_sam_demo import get_grounding_output, load_image, load_model
...
class SceneGraph():
    def __init__(self, map_resolution, map_size_cm, map_size, camera_matrix, is_navigation=True, agent=None) -> None:
        ...
        self.groundingdino_config_file = os.path.join(self.project_root, 'GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py')
        self.groundingdino_checkpoint = os.path.join(self.project_root, 'data/models/groundingdino_swint_ogc.pth')
        self.bert_base_uncased_path = os.path.join(self.project_root, 'data/models/bert-base-uncased')
        self.sam_version = 'vit_h'
        self.sam_checkpoint = os.path.join(self.project_root, 'data/models/sam_vit_h_4b8939.pth')
        self.segment2d_results = []
        self.max_detections_per_object = 10
        self.threshold_list = {'bathtub': 2, 'bed': 7, 'cabinet': 3, 'chair': 5, 'chest_of_drawers': 5, 'clothes': 9, 'fireplace': 4, 'gym_equipment': 7, 'picture': 9, 'plant': 3, 'seating': 2, 'shower': 2, 'sink': 3, 'sofa': 9, 'stool': 5, 'table': 8, 'toilet': 3, 'towel': 4, 'tv_monitor': 2, 'treadmill. fitness equipment.': 0}
        self.small_objects = ['bathtub', 'chest_of_drawers', 'plant', 'seating', 'shower', 'toilet', 'tv_monitor']
        self.found_goal_times_threshold = 1
        self.N_max = 10
        self.node_space = 'bathtub. bed. cabinet. chair. drawers. clothes.fireplace. gym. picture. plant. seating. shower. sink. sofa. stool. table. toilet. towel. tv. treadmill. fitness equipment.'
...
Object pair(s):
    ...
    def set_cfg(self):
        cfg = {'dataset_config': PosixPath(os.path.join(self.project_root, 'tools/replica.yaml')), 'scene_id': 'room0', 'start': 0, 'end': -1, 'stride': 5, 'image_height': 680, 'image_width': 1200, 'gsa_variant': 'none', 'detection_folder_name': 'gsa_detections_${gsa_variant}', 'det_vis_folder_name': 'gsa_vis_${gsa_variant}', 'color_file_name': 'gsa_classes_${gsa_variant}', 'device': 'cpu', 'use_iou': True, 'spatial_sim_type': 'overlap', 'phys_bias': 0.0, 'match_method': 'sim_sum', 'semantic_threshold': 0.5, 'physical_threshold': 0.5, 'sim_threshold': 1.2, 'use_contain_number': False, 'contain_area_thresh': 0.95, 'contain_mismatch_penalty': 0.5, 'mask_area_threshold': 25, 'mask_conf_threshold': 0.95, 'max_bbox_area_ratio': 0.5, 'skip_bg': True, 'min_points_threshold': 16, 'downsample_voxel_size': 0.025, 'dbscan_remove_noise': True, 'dbscan_eps': 0.1, 'dbscan_min_points': 10, 'obj_min_points': 0, 'obj_min_detections': 3, 'merge_overlap_thresh': 0.7, 'merge_visual_sim_thresh': 0.8, 'merge_text_sim_thresh': 0.8, 'denoise_interval': 20, 'filter_interval': -1, 'merge_interval': 20, 'save_pcd': True, 'save_suffix': 'overlap_maskconf0.95_simsum1.2_dbscan.1_merge20_masksub', 'vis_render': False, 'debug_render': False, 'class_agnostic': True, 'save_objects_all_frames': True, 'render_camera_path': 'replica_room0.json', 'max_num_points': 512}
        cfg = DictConfig(cfg)
    ...
    def get_seg_info_with_caption(self):
        """
        Return list of objects with caption + bbox + center:
        [
            {
                "caption": "chair",
                "bbox": [x1, y1, x2, y2],
        ...
        """
        info = []
        if not hasattr(self, 'seg_xyxy'):
            return info

        for obj, (x1, y1, x2, y2) in zip(self.objects_post, self.seg_xyxy):
            # bounding box
            cx = (x1 + x2) / 2.0
            cy = (y1 + y2) / 2.0

            # caption: objects_post 里一定有 'captions'
    ...
    def get_sam_mask_generator(self, variant:str, device) -> SamAutomaticMaskGenerator:
        if variant == "sam":
            ...
            model = load_model(self.groundingdino_config_file, self.groundingdino_checkpoint, self.bert_base_uncased_path, device=device)
    ...
    def compute_clip_features(self, image, detections, clip_model, clip_preprocess, clip_tokenizer, classes, device):
        ...
        for idx in range(len(detections.xyxy)):
            ...
            preprocessed_image = clip_preprocess(cropped_image).unsqueeze(0).to("cpu")

            crop_feat = clip_model.encode_image(preprocessed_image)
            crop_feat /= crop_feat.norm(dim=-1, keepdim=True)
            
            class_id = detections.class_id[idx]
            tokenized_text = clip_tokenizer([classes[class_id]]).to("cpu")
    ...
    def get_pose_matrix(self):
        ...
        # t = (self.observations['compass'] - np.pi / 2) # input degrees and meters
