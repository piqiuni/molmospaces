...
def evaluate_frame(detections: list[Mapping[str, Any]], *, graph: Mapping[str, Any] | None = None, thresholds: Mapping[str, float] | None = None) -> dict[str, Any]:
    ...
    for det in detections:
        label = det.get("semantic_class", det.get("class", "")); node = next((n for n in nodes if n.get("label") == label or n.get("semantic_class") == label), None); reports.append(evaluate_detection(det, map_node=node, thresholds=thresholds))
