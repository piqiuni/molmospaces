...
inline void GridSlamProcessor::scanMatch(const double* plainReading){
  ...
  for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
    const OrientedPoint odometryPrediction=it->pose;
    OrientedPoint corrected;
    double score, l, s;
    score=m_matcher.optimize(corrected, it->map, it->pose, plainReading);
    //    it->pose=corrected;
    if (score>m_minimumScore){
      if (m_lockScanMatchingYaw){
        corrected.theta=odometryPrediction.theta;
      }
      if (m_maxScanMatchingTranslationCorrection>0.){
        const double dx=corrected.x-odometryPrediction.x;
        const double dy=corrected.y-odometryPrediction.y;
        const double correction=sqrt(dx*dx+dy*dy);
        if (correction>m_maxScanMatchingTranslationCorrection){
          const double scale=m_maxScanMatchingTranslationCorrection/correction;
          corrected.x=odometryPrediction.x+dx*scale;
          corrected.y=odometryPrediction.y+dy*scale;
        }
      }
      it->pose=corrected;
    } else {
