...
using namespace std;
  ...
  GridSlamProcessor::GridSlamProcessor(): m_infoStream(cout){
    ...
    m_minimumScore=0.;
    m_useOdometryPose=false;
    m_lockScanMatchingYaw=false;
    m_maxScanMatchingTranslationCorrection=0.;
  }
  ...
  GridSlamProcessor::GridSlamProcessor(const GridSlamProcessor& gsp) 
    ...
    m_useOdometryPose=gsp.m_useOdometryPose;
    m_lockScanMatchingYaw=gsp.m_lockScanMatchingYaw;
    m_maxScanMatchingTranslationCorrection=gsp.m_maxScanMatchingTranslationCorrection;
...
(double srr, double srt, double str, double stt){
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles){
	...
	if (m_useOdometryPose){
	  m_neff=static_cast<double>(m_particles.size());
	  for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
