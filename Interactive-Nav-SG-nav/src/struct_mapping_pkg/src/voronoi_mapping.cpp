...
void VoronoiMapping::occupyCell(int x, int y) {
  gridMap[x][y] = 1;
  setObstacle(x,y);
}
void VoronoiMapping::clearCell(int x, int y) {
  gridMap[x][y] = 0;
  removeObstacle(x,y);
}

void VoronoiMapping::setObstacle(int x, int y) {
  dataCell c = data[x][y];
...
void VoronoiMapping::removeObstacle(int x, int y) {
  dataCell c = data[x][y];
...
void VoronoiMapping::exchangeObstacles(std::vector<INTPOINT> points) {
...
void VoronoiMapping::update(bool updateRealDist) {
...
float VoronoiMapping::getDistance( int x, int y ) {
  if( (x>0) && (x<sizeX) && (y>0) && (y<sizeY)) return data[x][y].dist; 
  else return -INFINITY;
}

bool VoronoiMapping::isVoronoi( int x, int y ) {
  dataCell c = data[x][y];
  return (c.voronoi==free || c.voronoi==voronoiKeep);
}


void VoronoiMapping::commitAndColorize(bool updateRealDist) {
  // ADD NEW OBSTACLES
...
void VoronoiMapping::checkVoro(int x, int y, int nx, int ny, dataCell& c, dataCell& nc) {

  if ((c.sqdist>1 || nc.sqdist>1) && nc.obstX!=invalidObstData) { 
    if (abs(c.obstX-nc.obstX) > obstacle_distance_threshold_ || abs(c.obstY-nc.obstY) > obstacle_distance_threshold_) {
...
void VoronoiMapping::reviveVoroNeighbors(int &x, int &y) {
  for (int dx=-1; dx<=1; dx++) {
    int nx = x+dx;
    if (nx<=0 || nx>=sizeX-1) continue;
    for (int dy=-1; dy<=1; dy++) {
...
bool VoronoiMapping::isOccupied(int x, int y) {
  dataCell c = data[x][y];
  return (c.obstX==x && c.obstY==y);
}

bool VoronoiMapping::isOccupied(int &x, int &y, dataCell &c) { 
  return (c.obstX==x && c.obstY==y);
}

void VoronoiMapping::visualize(const char *filename) {
  // write pgm files
...
VoronoiMapping::markerMatchResult VoronoiMapping::markerMatch(int x, int y) {
  ...
  // 4-connected patterns
  // L形和T形连接：保持分支点
  // 模式：角落空，相邻两个方向有Voronoi点
  // f[0]=左上, f[1]=上, f[2]=右上, f[3]=左, f[4]=右, f[5]=左下, f[6]=下, f[7]=右下
  if ((!f[0] && f[1] && f[3]) || (!f[2] && f[1] && f[4]) || (!f[5] && f[3] && f[6]) || (!f[7] && f[6] && f[4])) return keep;
  
  // 横线模式：左右有，上下空 (f[3] && f[4] && !f[1] && !f[6])
  // 竖线模式：上下有，左右空 (f[1] && f[6] && !f[3] && !f[4])
