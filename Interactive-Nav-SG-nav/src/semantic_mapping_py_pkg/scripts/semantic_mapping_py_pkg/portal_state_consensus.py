...
class PortalStateConsensus:
    ...
    def __init__(
        ...
        *,
        ...
        self.cooldown_s = None if cooldown_s is None else max(0.0, float(cooldown_s))
        if self.cooldown_s is not None:
            self.cooldown_steps = int(math.ceil(self.cooldown_s * 1000.0))
    ...
    def _clock_step(self, capture_step: int | None) -> int | None:
        # Physical cooldown is elapsed time, independent of camera FPS or stalls.
        return int(time.monotonic() * 1000.0) if self.cooldown_s is not None else capture_step
    ...
    def record_authoritative(
        ...
        *,
        ...
        capture_step = self._clock_step(capture_step)
    ...
    def _result(
        ...
        *,
        ...
        return {
            ...
            "cooldown_until_step": int(cooldown_until_step) if self.cooldown_s is None else -1,
            **({"cooldown_clock": "monotonic", "cooldown_until_monotonic_s": cooldown_until_step / 1000.0}
