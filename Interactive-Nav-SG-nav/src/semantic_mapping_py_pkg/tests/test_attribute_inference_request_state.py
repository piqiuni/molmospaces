...
def test_room_request_signature_and_failure_refresh_cooldown(monkeypatch) -> None:
    ...
    assert node._try_reserve_room("room_1", "same") is None
    assert node._try_reserve_room("room_1", "new_members") is not None
    node.room_pending.clear()
    ...
    assert node._room_signature(1, objects) == node._room_signature(1, list(objects))
...
def test_failed_room_call_uses_rule_fallback_and_waits_for_refresh():
    ...
    node.room_pending = {"room_1": {
        "episode_id": "episode", "signature": "members",
    ...
    node.room_failure_refresh_interval_s = 30.0
    node.room_success_refresh_interval_s = 120.0
    node.room_request_timeout_s = 30.0
    ...
    node._infer_room(
        ...
        signature="members", generation=0, request_sequence=1,
        enqueued_at=started, deadline_monotonic=started + 30.0,
    )
    ...
    assert node._try_reserve_room("room_1", "members") is None
    node.room_failures["room_1"]["failed_at"] -= 31.0
    node.room_last_request["room_1"] -= 3.0
    assert node._try_reserve_room("room_1", "members") is not None
...
def test_failed_m1_calls_preserve_new_views_and_targeted_reserve() -> None:
    node = _budget_test_node()
    calls = []
    ...
    assert len(calls) == 8
    assert node.m1_call_counts["fridge"] == 8
    assert node._try_reserve("fridge", "ninth-passive-view") is None
    for index in (9, 10):
        targeted_refresh = {"request_id": f"targeted-{index}"}
        reservation = node._force_reserve_targeted_refresh(
            "fridge", f"targeted-{index}", "episode_1", targeted_refresh,
        )
        assert reservation is not None
        node._infer(
            object_id="fridge", detection={"name": "fridge"},
            visual_evidence=np.zeros((40, 60, 3), dtype=np.uint8),
            episode_id="episode_1", frame_id=str(index),
            image_sequence=index, stamp=10.0,
            signature=f"targeted-{index}", enqueued_at=time.monotonic(),
            targeted_refresh=targeted_refresh, **reservation,
        )
    assert len(calls) == 10
    assert node.m1_call_counts["fridge"] == 10
    assert node._force_reserve_targeted_refresh(
        "fridge", "eleventh-view", "episode_1", {"request_id": "exhausted"},
    ) is None

def test_success_does_not_reset_m1_call_count() -> None:
    node = _budget_test_node()
...
def test_queue_deadline_expiry_does_not_charge_calls_or_block_new_views() -> None:
    node = _budget_test_node()
    updates = []
...
def test_m1_call_budget_resets_on_episode_change() -> None:
    node = _budget_test_node()
    node.m1_call_counts["fridge"] = 10
