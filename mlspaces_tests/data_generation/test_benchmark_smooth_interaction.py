...
def test_public_target_stops_drawer_without_claiming_episode_success(monkeypatch):
    class Controller:
        def __init__(self, **kwargs):
            self._events = []
        def enqueue_command(self, command):
        ...
        def before_step(self, task, index):
            pass
        def after_step(self, task, index):
            raise AssertionError("must retain open drawer")
    monkeypatch.setattr(smooth_interaction, "AtomicForceInteractionController", Controller)
    execution = smooth_interaction.run_smooth_interaction(
        object(), {}, object_name="opaque", step=lambda controller, index: "public_target_visible")
    assert execution["result"]["stopped_on_public_target"]
    assert not execution["result"]["interrupted_by_goal_status"]
...
@pytest.mark.parametrize("public_evidence", [False, True])
def test_smooth_target_discovery_keeps_group_index_for_result_serialization(monkeypatch, public_evidence):
    ...
    def run(task, command, *, object_name, step):
        ...
        for index in range(1, 11):
            expected = "public_target_visible" if public_evidence and index == 10 else index == 10
            assert step(controller, index) == expected
    ...
    if public_evidence:
        runtime.goal_evidence = SimpleNamespace(frames=[SimpleNamespace(capture_step=1)])
    ...
    if public_evidence:
        assert not hasattr(runtime, "pending_goal_terminal")
    else:
