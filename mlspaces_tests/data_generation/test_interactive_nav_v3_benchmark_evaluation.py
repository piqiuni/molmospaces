...
def test_empty_resolved_interaction_ids_falls_back_to_singular_id() -> None:
    assert benchmark_runner._resolved_interaction_ids(
        {
            "resolved_interaction_ids": [],
            "resolved_interaction_id": "open_drawer_0",
        }
    ) == ["open_drawer_0"]


def test_external_policy_observation_excludes_private_gt_and_is_isolated() -> None:
    source = {
        "head_camera": np.zeros((2, 2, 3), dtype=np.uint8),
        "head_camera_depth": np.ones((2, 2), dtype=np.float32),
        "sensor_param_head_camera": {"intrinsic_cv": np.eye(3)},
        "robot_base_pose": np.zeros(7),
        "qpos": np.zeros(25),
        "env_states": {"qpos": np.ones(100)},
        "pickup_obj_pose": {"secret_target": np.ones(7)},
        "actions/commanded_action": np.ones(3),
    }

    # Runtime observations are batched even when the evaluator owns one scene.
    public = benchmark_runner._public_policy_observation([source], _public_episode())

    assert set(public) == {
        "head_camera",
        "head_camera_depth",
        "sensor_param_head_camera",
        "robot_base_pose",
        "qpos",
    }
    public["head_camera"][0, 0, 0] = 255
    public["head_camera_depth"][0, 0] = 9.0
    public["sensor_param_head_camera"]["intrinsic_cv"][0, 0] = 99.0
    assert source["head_camera"][0, 0, 0] == 0
    assert source["head_camera_depth"][0, 0] == 1.0
    assert source["sensor_param_head_camera"]["intrinsic_cv"][0, 0] == 1.0
...
def test_private_target_precheck_is_preserved_only_for_compatibility_policies(
    ...
    episode: dict[str, object] = {
        "interactive_nav": {"interaction_requirement": "unnecessary"}
    }
...
def test_required_target_precheck_waits_for_interaction_completion(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        benchmark_runner,
        "target_metrics",
        lambda _task, _episode: (True, 0.5, 0.25),
    )
    ...
    monkeypatch.setattr(benchmark_runner, "score_interactions", lambda *_args: incomplete)
    task = SimpleNamespace(env=object())
    episode = {"interactive_nav": {"interaction_requirement": "required"}}
    assert not benchmark_runner._compatibility_target_precheck(
        task,
        episode,
        restricted_public_mode=False,
        transient_target_discovery=None,
    ...
    monkeypatch.setattr(benchmark_runner, "score_interactions", lambda *_args: complete)
    assert benchmark_runner._compatibility_target_precheck(
        task,
        episode,
        restricted_public_mode=False,
...
def test_empty_plural_interaction_ids_fall_back_to_singular(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "scripts.InteractiveNav.evaluation.benchmark_metrics.joint_open_fraction",
        lambda _env, _interaction: 1.0,
    )
    interaction_id = "required-door"
    episode = {
        "interactive_nav": {
            "interaction_requirement": "required",
            "interactions": [
    ...
    assert score.required_interaction_success
    assert score.sequence_success
