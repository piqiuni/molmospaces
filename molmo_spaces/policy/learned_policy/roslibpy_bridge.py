"""rosbridge transport that preserves the native ROS navigation topic interface.

This module runs in the MolmoSpaces Conda environment and requires only
``roslibpy``; ROS itself can run in a separate RoboStack Conda environment.
"""
...
class RoslibpyBridgeClient:
    ...
    def __init__(
        ...
        *,
        ...
        if roslibpy is None:
            raise ImportError(
                "RoslibpyBridgeClient requires roslibpy. Install it with "
        ...
        self._pointcloud_pub = roslibpy.Topic(
            self.ros, pointcloud_topic, "sensor_msgs/PointCloud2"
        )
        self._reset_pub = roslibpy.Topic(self.ros, reset_topic, "std_msgs/Empty")
        self._cmd_vel_sub = roslibpy.Topic(
            self.ros, cmd_vel_topic, "geometry_msgs/TwistStamped"
        )
    ...
    def publish_odom(
        ...
        *,
        ...
        self._odom_pub.publish(
            roslibpy.Message(
                {
                    "header": _header(frame_id, stamp_sec),
            ...
            roslibpy.Message(
                {
                    "transforms": [
                        {
                            "header": _header(parent_frame, stamp_sec),
    ...
    def _on_move_base_status(self, message: dict[str, Any]) -> None:
        active = any(
            int(status.get("status", -1)) == 1 for status in message.get("status_list", [])
        )
        with self._lock:
