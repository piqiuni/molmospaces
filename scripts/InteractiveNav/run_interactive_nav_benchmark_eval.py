...
Example (a small smoke run)::
    ...
    python scripts/InteractiveNav/run_interactive_nav_benchmark_eval.py \
      ...
      --episodes-per-domain 1 --workers 1 --max-steps 1 \
      --policy factory \
...
def _episode_sr(rows: list[dict[str, Any]]) -> float | None:
    """Formal navigation SR on scoring-eligible rows.

    ``scoring_eligible`` is the denominator contract: an eligible row with a
    malformed/missing navigation-success field is still a failed episode,
...
def _episode_nav_sr(rows: list[dict[str, Any]]) -> float | None:
    """Backward-compatible explicit alias for the formal SR."""

    return _episode_sr(rows)


def _episode_interaction_conditioned_sr(rows: list[dict[str, Any]]) -> float | None:
    eligible = [row for row in rows if bool(row.get("scoring_eligible", True))]
    if not eligible:
        return None
...
def _planned_success_rate(rows: list[dict[str, Any]], planned_count: int | None = None) -> float | None:
    """Return navigation success/plan, counting exceptions or missing rows as failures."""

    denominator = len(rows) if planned_count is None else int(planned_count)
    if denominator <= 0:
        return None
    successes = sum(
        bool(row.get("scoring_eligible", True))
        and bool(row.get("nav_success", row.get("task_success", False)))
        for row in rows
    )
...
def _domain_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for domain in DOMAIN_NAMES:
        ...
        by_domain[domain] = {
            ...
            "interaction_conditioned_sr": _episode_interaction_conditioned_sr(domain_rows),
...
def _rate(rows: list[dict[str, Any]], key: str, fallback: str) -> float | None:
    values = [
        row.get(key, row.get(fallback))
        for row in rows
        if bool(row.get("scoring_eligible", True))
    ]
...
def _write_summary_csv(path: Path, rows: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    overall = {
        ...
        "interaction_conditioned_sr": _episode_interaction_conditioned_sr(rows),
...
class _Progress:
    ...
    def update(self, row: dict[str, Any]) -> None:
        ...
        snapshot = {
            ...
            "interaction_conditioned_sr": _episode_interaction_conditioned_sr(self.rows),
...
def _build_schedule(args: argparse.Namespace) -> tuple[list[ScheduledEpisode], dict[str, Any]]:
    ...
    manifest = {
        "schema_version": "interactive_nav_v3_mixed_eval_v2",
        "wrapper_implementation_sha256": _sha256(Path(__file__).resolve()),
        "protocol_version": benchmark_runner.PROTOCOL_VERSION,
        "public_observation_schema_version": (
            benchmark_runner.PUBLIC_OBSERVATION_SCHEMA_VERSION
        ),
        ...
        "allow_runtime_ineligible": bool(args.allow_runtime_ineligible),
        "pinned_assets": {
            "path": str(Path(args.pinned_assets_file).expanduser().resolve()),
            "sha256": _sha256(Path(args.pinned_assets_file).expanduser().resolve()),
            "versions": json.loads(
                Path(args.pinned_assets_file)
                .expanduser()
                .resolve()
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--pinned-assets-file",
        type=Path,
        default=DEFAULT_PINNED_ASSETS_FILE,
        help="Asset version override (default: versions bundled with the release).",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Return success even when runtime compatibility excludes an episode; "
            "intended only for auditing custom benchmarks."
        ),
    )
    parser.add_argument(
        "--dry-run",
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Reuse a caller-provided, large-volume NLTK cache when available.  The
    # output-local default keeps clean runs off /root and /tmp.
    os.environ.setdefault("NLTK_DATA", str(runtime_cache / "nltk"))
    ...
    pinned_assets_file = Path(args.pinned_assets_file).expanduser().resolve()
    if not pinned_assets_file.is_file():
        raise FileNotFoundError(f"Missing pinned assets file: {pinned_assets_file}")
    os.environ["MLSPACES_PINNED_ASSETS_FILE"] = str(pinned_assets_file)
    for cache_path in (
        runtime_cache / "xdg",
        runtime_cache / "hf",
        runtime_cache / "torch",
    ...
    summary.update(
        {
            ...
            "exception_count": sum(row.get("status") == "exception" for row in rows),
            # Count ordinary navigation failures as well as runtime exceptions
            # and missing worker rows; SR=0 must not be reported as "no failures".
            "failed_or_incomplete_episode_count": sum(
                not bool(row.get("scoring_eligible", True))
                or not bool(row.get("nav_success", row.get("task_success", False)))
                for row in rows
            ) + max(0, len(schedules) - len(rows)),
            "interaction_failed_or_incomplete_episode_count": sum(
                not bool(row.get("scoring_eligible", True))
                or not bool(row.get("success", False))
                for row in rows
            ) + max(0, len(schedules) - len(rows)),
            "sr": _episode_sr(rows),
            ...
            "wrapper_implementation_sha256": manifest[
                "wrapper_implementation_sha256"
            ],
    ...
    # Ordinary policy failures are valid scored outcomes. Infrastructure,
    # scene, or policy exceptions make the batch incomplete and must be visible
    # to shell/CI callers as a failed command.
    runtime_ineligible = len(rows) - summary["scoring_eligible_episode_count"]
    return (
        1
        if summary["exception_count"]
        or len(rows) != len(schedules)
        or (runtime_ineligible and not args.allow_runtime_ineligible)
        else 0
    )
