...
The default benchmark root is the frozen local release documented in
``docs/current_original_interactive_nav.md``.  Pass explicit ``--*-benchmark``
paths when evaluating another release.
"""
...
def _read_episodes(path: Path) -> list[dict[str, Any]]:
    ...
    if not all(isinstance(item, dict) for item in payload):
        raise ValueError(f"Every episode in {path} must be a JSON object")
...
def _domain_path_args(args: argparse.Namespace) -> dict[str, Path]:
    ...
    values = {
        "channel": Path(args.channel_benchmark).expanduser() if args.channel_benchmark else root / "channel.json",
        "container": Path(args.container_benchmark).expanduser() if args.container_benchmark else root / "container.json",
        "mixed": Path(args.mixed_benchmark).expanduser() if args.mixed_benchmark else root / "mixed.json",
    }
