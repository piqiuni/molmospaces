...
body.dark .page { width:100vw; height:100vh; min-height:0; max-height:100vh; padding:10px 14px 8px; gap:7px; }
body.dark header { min-height:38px; padding:0 1px 2px; }
body.dark .logo { display:none; }
body.dark .brand { gap:0; }
body.dark h1 { font-size:22px; line-height:1.05; text-shadow:0 0 18px #58baff28; }
body.dark .subtitle { font-size:9px; letter-spacing:.01em; }
body.dark .top-tags { gap:7px; }
body.dark .tag { padding:5px 9px; border-radius:6px; background:linear-gradient(180deg,#102b47,#0b1828); box-shadow:inset 0 1px #ffffff0b,0 0 9px #0c6aa522; font-size:9px; }
body.dark .layout { grid-template-columns:minmax(0,1.86fr) minmax(390px,1fr); gap:12px; }
body.dark .visuals { grid-template-rows:minmax(0,1.08fr) minmax(0,1fr); gap:7px; }
body.dark .bottom { gap:7px; }
...
body.dark .panel-title { height:30px; padding:6px 10px; color:#70c5ff; font-size:12px; letter-spacing:.015em; border-bottom-color:#214a72; background:linear-gradient(100deg,#102d4dcc 0%,#0d233900 70%); position:relative; }
...
body.dark .rail { grid-template-rows:minmax(230px, 3fr) minmax(0,7fr); gap:7px; }
...
body.dark .robot-head h2, body.dark .agent h2 { font-size:14px; color:#78c7ff; }
...
body.dark .timeline { padding:2px 3px; overflow-x:hidden; overflow-y:auto; }
body.dark .eventtext, body.dark .eventtext span, body.dark .eventtext b { min-width:0; max-width:100%; overflow-wrap:anywhere; word-break:break-word; }
...
body.dark .eventtext { font-size:11px; }
body.dark .eventtext b { font-size:12px; }
body.dark footer { height:48px; margin-top:0; background:linear-gradient(90deg,#0b1a2b,#10243a,#0b1a2b); border-color:#2b5f91; gap:10px; font-size:10px; }
...
body.dark .dog { display:block; width:48%; max-width:48%; height:auto; max-height:none; justify-self:center; align-self:center; }
...
.agent{{padding:12px;display:grid;grid-template-rows:auto minmax(105px,.45fr) minmax(90px,.38fr) minmax(180px,1.17fr);gap:9px}}.block{{min-height:0;border:1px solid var(--line);border-radius:9px;background:var(--panel2);padding:9px}}.block h3{{font-size:12px;margin:0 0 7px;color:var(--text)}}.calls{{display:grid;gap:7px}}.call{{display:grid;grid-template-columns:32px 1fr auto;gap:7px;align-items:center;padding:7px;border:1px solid var(--line);border-radius:8px;background:var(--panel2);font-size:9px}}.stage{{font-weight:800;color:var(--blue)}}.result{{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--text)}}.latency{{color:var(--teal)}}.note{{font-size:8px;color:var(--muted);margin-top:5px}}.candidate-list{{display:grid;gap:4px;max-height:100%;overflow:auto}}.candidate{{display:grid;grid-template-columns:minmax(0,1.1fr) minmax(0,1.5fr) auto;gap:6px;align-items:center;padding:5px 7px;border:1px solid var(--line);border-radius:6px;background:var(--panel2);font-size:10px;min-width:0}}.candidate.selected{{border-color:var(--gold);box-shadow:0 0 0 1px color-mix(in srgb,var(--gold) 35%,transparent)}}.candidate-id,.candidate-name{{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}.candidate-id{{color:var(--blue)}}.candidate-name{{color:var(--text)}}.candidate-kind{{color:var(--teal);font-size:9px;white-space:nowrap}}
...
body.dark .debug-call-thumb { display:block; width:100%; height:auto; aspect-ratio:16/9; object-fit:contain; border:1px solid #35516d; border-radius:5px; background:#070d15; }
...
function darkIsRoomM1(event) { if (event?.m1_input_image_key || event?.object_id || event?.m1_input_label) return false; let raw=event?.raw_text??event?.response?.raw_text??event?.payload?.result??''; try { const value=typeof raw==='string'?JSON.parse(raw):raw; return !!(value&&('room_attribute' in value||'room_id' in value)); } catch (_) { return false; } }
function darkCallObjectName(event) {
  if (darkIsRoomM1(event)) return '房间属性';
...
function darkCallQuestion(event, stage) {
  if (stage === 'M1') { const raw=String(event?.m1_input_label||event?.context?.semantic_class||event?.target_kind||'').toLowerCase(); if(raw==='locker') return '判断图中标注的 locker 是否为冰箱，给出冰箱置信度，并识别其开合状态。'; return '判断图中的' + darkCallObjectName(event) + '是否可交互，并识别其开合状态。'; }
...
function darkCallResult(event) {
  ...
  try {
    ...
    if (value.room_attribute || value.room_id) return clip([value.room_attribute || 'unknown', value.confidence != null ? '置信度 ' + Number(value.confidence).toFixed(2) : ''].filter(Boolean).join(' · '), 110);
    const choice = value.observed_object_name || value.semantic_name || value.object_name || value.ranked_ids || value.candidate_id || value.label || value.interaction_class || value.coarse_state || value.state || (value.success === true ? '验证通过' : value.success === false ? '验证失败' : '');
    const name=value.observed_object_name||value.semantic_name||value.object_name||value.label||'';const category=value.interaction_class||value.semantic_class||'';const state=value.coarse_state||value.state||'';const confidence=value.fridge_confidence??value.refrigerator_confidence??value.is_refrigerator_confidence??value.confidence??value.score;return clip([name|| (Array.isArray(choice)?choice.join(' → '):choice),category,state,confidence!=null?'置信度 '+Number(confidence).toFixed(2):''].filter(Boolean).join(' · '), 110);
  } catch (_) {
...
function darkM1Thumb(event, index) {
  ...
  const kind=String(event?.target_kind||event?.context?.target_kind||event?.module||'').toLowerCase();
  if (!key || kind.includes('room') || darkIsRoomM1(event)) return null;
...
renderCalls = function(state) {
  ...
  events.forEach((event,index)=>{
    ...
    if(stage==='M1'){const thumb=darkM1Thumb(event,index);if(thumb)left.append(thumb);darkCallChips(left,[darkCallObjectName(event)],1)}
