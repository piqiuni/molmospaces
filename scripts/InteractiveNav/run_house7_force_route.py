...
class StagedRouteExecutor:
    ...
    def run(self, route: dict[str, Any]) -> dict[str, Any]:
        ...
        target_root = str(route["interaction"]["source_object_name"])
        started = time.monotonic()
        self.result = {"route_id": route_id, "status": "RUNNING"}
        self._record("route_started", route_id=route_id, target_root=target_root)
        try:
            ...
            command = {
                ...
                "source_object_name": target_root,
                "action": "open",
    ...
    def wait_for_portal_state(
        ...
        with self._condition:
            while True:
                ...
                if snapshot is not None and snapshot.get("state") == expected_state:
                    return snapshot
                remaining = deadline - time.monotonic()
                if remaining <= 0.0:
                    raise TimeoutError(
