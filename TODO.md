...
### 2026-09-25 实物 addon 分离

- [x] 分离前存档 `5619e193`，保留 exp-setting 合并结果。
- [x] 实物配置/launch 单一来源，ROS 包作为薄接入外壳。
- [x] 抽取进度时钟、门接近几何、语义证据判定与共用采集几何。
- [x] watchdog 改读 ROS 进度，关闭网页仍可监督主链。
- [x] 第二阶段抽取开门后地图新鲜度策略与门外观察 tick 状态机；保留执行器唯一归属。
- [x] 第三阶段抽取容器两阶段纯策略、M1 证据合同与交互命令组装；全量离线回归 2041 项通过。
- [ ] 将公共修复按主题反向合入 exp-setting；继续分解 M1 请求/证据缓存与导航执行编排。
- [ ] 安全现场验证无网页/开启网页时的数据频率和延迟；本次不重启真机服务。

边界与后续合并规则见 `docs/physical_addon_architecture.md`，命令见 `test.md`。
